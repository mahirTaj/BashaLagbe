// frontend/src/pages/MessageInterface.jsx
...
export default function MessageInterfacePage() {
  ...
  // Update thread list previews in real-time
  useEffect(() => {
    const socket = getSocket();
    const onNotify = ({ threadId: tid, preview }) => {
      setThreads(prev => {
        ...
        if (idx >= 0) {
          next[idx] = { ...next[idx], lastMessage: preview, updatedAt: new Date().toISOString() };
          // move to top
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] border rounded-lg overflow-hidden bg-white">
        ...
        <ul>
          {threads.map(t => (
            <li key={t._id}>
              <Link
                to={`/messages/${t._id}`}
                className={`block p-3 hover:bg-gray-50 ${String(t._id) === String(threadId) ? 'bg-indigo-50' : ''}`}
              >
                <div className="text-sm font-medium text-gray-800">{t?.listing?.title || 'Listing'}</div>
      ...
      <main className="flex-1">
        {active ? (
          <ChatBox threadId={threadId} initialThread={active.thread} initialMessages={active.messages} />
        ) : (
          <div className="h-full flex items-center justify-center text-gray-500">Select a conversation</div>
        )}
      </main>
    </div>
  );
